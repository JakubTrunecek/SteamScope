...
export function AchievementCollection({ id, games }: { id: string; games: Game[] }) {
  const [rows, setRows] = useState<ScannedGame[]>([]);
  ...
  return <section className="card section"><h2>Achievements across your library</h2>
    ...
    <p className="muted">Results cover only loaded games and stay on this page until you leave or reload. This is achievement activity, not hours played. Dates use UTC.</p>
    ...
    <div className="result-actions"><button disabled={running || rows.length === games.length} onClick={() => void scan()}>{rows.length ? 'Load next 5 games' : 'Explore achievements'}</button>{running && <button onClick={() => { active.current?.abort(); setMessage('Stopped. Completed games are kept; continue whenever you like.'); }}>Stop loading</button>}</div>
    ...
    {!!rows.length && <><p><strong>{insights.unlocked}</strong> unlocked achievements in loaded games · {insights.undated} without a usable date</p>
      ...
      <h3>Unlock calendar</h3>{months.length ? <><label htmlFor="unlock-month">Month (UTC)</label><select id="unlock-month" value={selectedMonth} onChange={event => setMonth(event.target.value)}>{months.map(value => <option key={value}>{value}</option>)}</select><div className="unlock-calendar">{Array.from({ length: new Date(Date.UTC(Number(selectedMonth.slice(0, 4)), Number(selectedMonth.slice(5)), 0)).getUTCDate() }, (_, index) => { const date = `${selectedMonth}-${String(index + 1).padStart(2, '0')}`; const count = insights.days.find(([day]) => day === date)?.[1] ?? 0; return <div className={count ? 'calendar-day active' : 'calendar-day'} key={date}><span>{index + 1}</span><strong>{count}</strong><span className="muted">unlocks</span></div>; })}</div><p className="muted">Zero means no dated unlocks in the loaded results for that day. Missing dates are excluded.</p></> : <p>No dated unlocks available for the calendar.</p>}
    </>}
  </section>;
}
