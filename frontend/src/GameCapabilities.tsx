...
function Achievements({ id, appId, schema }: { id: string; appId: string; schema: GameSchema | null }) {
  ...
  const filtered = sortAchievements(result?.filter(item => (selectedName === null || item.name === selectedName) && (rarity === 'all' || (rarity === 'unknown' ? !percentages.has(item.name) : percentages.has(item.name) && percentages.get(item.name)! <= Number(rarity))) && (filter === 'all' || item.achieved === (filter === 'unlocked')) && `${item.name} ${metadata.get(item.name)?.displayName ?? ''}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())) ?? [], order, percentages, metadata);
  return <section className="card section" aria-labelledby="achievements-title"><h2 id="achievements-title" tabIndex={-1}>Achievements</h2>
    {state.status === 'loading' ? <p role="status">Loading achievements…</p>
      ...
      : <><p><strong>{unlocked} / {result?.length}</strong> returned achievements unlocked in this game</p>
        ...
        <AchievementHighlights items={result ?? []} rates={percentages} metadata={metadata} onSelect={name => { setSelectedName(name); setQuery(''); setFilter('all'); setRarity('all'); setShown(30); }} /><div className="library-tools section"><div><label htmlFor="achievement-search">Find an achievement</label><input id="achievement-search" value={query} onChange={event => { setQuery(event.target.value); setSelectedName(null); setShown(30); }} placeholder="Search names" /></div>
