...
export function createWorker(upstream: typeof fetch = fetch, now = Date.now) {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      const cacheKey = communityMatch || resource === 'schema' || resource === 'global' ? `${resource}:${appId}` : `${resource}:${steamId}:${appId ?? ''}`;
      ...
      if (!operation) {
        ...
        operation = (async () => {
          ...
          const body = resource === 'players' || resource === 'reviews'
            ? await fetchCommunity(resource, appId!, upstream, now)
            : await fetchSteam(resource, steamId, env.STEAM_API_KEY!, upstream, appId);
