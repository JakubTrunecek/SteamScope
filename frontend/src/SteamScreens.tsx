...
export function Library({ id }: { id: string }) {
  ...
  return <><LibraryInsights games={state.data.games} id={id} compact /><section className="card section">
    ...
    <div className="review-discovery"><h2>Discover by community reviews</h2><p className="muted">Choose a playtime group above, then choose how many matching games to load. Larger selections can take several minutes. Keep this page open; leaving stops loading. Review filters below apply to loaded data only; loading still covers games matching your name and playtime filters. A running batch keeps the selection it started with.</p><div className="library-tools"><div><label htmlFor="review-threshold">Positive reviews</label><select id="review-threshold" value={rating} onChange={event => setRating(event.target.value)}><option value="all">Any / not loaded</option><option value="0">Any known percentage</option><option value="80">At least 80%</option><option value="90">At least 90%</option><option value="95">At least 95%</option></select></div><div><label htmlFor="review-minimum">Minimum review count</label><select id="review-minimum" value={minimum} onChange={event => setMinimum(Number(event.target.value))}><option value={0}>No minimum</option><option value={100}>100 reviews</option><option value={1000}>1,000 reviews</option><option value={10000}>10,000 reviews</option></select></div></div><ScanSize value={reviewSize} onChange={setReviewSize} running={community.running} label="Review scan size" /><ScanProgress progress={community.progress} /><div className="result-actions"><button disabled={community.running || checked === candidates.length} onClick={() => void community.load(candidates, reviewSize)}>{reviewSize === 'all' ? 'Load reviews for all remaining games' : `Load reviews for next ${reviewSize} games`}</button>{community.running && <button onClick={community.stop}>Stop loading reviews</button>}<button onClick={() => { setRating('all'); setMinimum(0); }}>Reset review filters</button></div><p role="status">{checked} of {candidates.length} matching games checked for reviews</p><p role="status">{community.message}</p><p className="muted">All languages and purchase types; Steam filters off-topic activity. Percentages are not adjusted for sample size. Equal percentages sort by review count; unknown percentages sort last. All loaded results stay on this page. The last 500 public game summaries remain available when navigating in this tab; reload clears them.</p></div>
